...
impactMetrics:
  - value: Rolling out across Europe
    label: After a strong test-market reception
    description: Used by the media planners who decide where Mercedes-Benz's ad spend goes.
  - value: "⁓80%"
    label: Fewer settings to enter per scenario
    description: Each setting is entered once globally, rather than in every tool.
