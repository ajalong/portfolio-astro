...
impactMetrics:
  - value: Model any scenario
    label: Before committing budget
    description: Spend and outcome update live across all seven channels.
  - value: "⁓80%"
    label: Less parameter entry per scenario
    description: ⁓60 parameter entries per scenario, down to 12, by setting the global parameters once.
  - value: Global parameters
    label: Consistency across the platform
    description: Every tool reads from the single scenario model, so a decision rests on one consistent set of figures.
roleSummary: |
